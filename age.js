...
function validate() {
  ...
  inputs.forEach((i) => {
    ...
    if (!i.value) {
      ...
      validator = false;
    } else if (monthOtp.value > 12) {
      monthOtp.style.borderColor = "red";
      monthOtp.parentElement.querySelector("small").innerText =
        "must be a valid month";
      validator = false;
    } else if (dayOtp.value > 31) {
      dayOtp.style.borderColor = "red";
      dayOtp.parentElement.querySelector("small").innerText =
        "must be a valid day";
      validator = false;
    } else {
